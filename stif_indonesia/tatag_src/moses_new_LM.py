...
def run_pipeline(base_model_name="working/train/model/moses.ini", base_data_name = "data", 
                batch_data_dir = "data/unlabeled_batches/", batch_predictions_dir = "out_predict/batch_predicts_leipzig/", 
                logs_dir = "logs/", batch_name="batch", iters=10, eval_dir = "out_predict/batch_eval_leipzig/"):
    model = base_model_name
    data = base_data_name

    print("Evaluating base model")
    gold_prediction_path = f"{eval_dir}data_test_predict_supervised.tgt"
    gold_result_path = f"{eval_dir}eval"

    predict_moses(f"working/train/model/moses.ini", "data/data_test.src", gold_prediction_path)
    bleu_calculate(gold_prediction_path, "data/data_test.tgt", f"{gold_result_path}_leipzig.txt")
    for i in range(iters):
        ...
        with open(f"{logs_dir}leipzig/log{i}.txt","ab") as log:
            try:
        ...
        subprocess.run([f"cp -R working/train/model saved_models/leipzig/batch_{i}"], shell=True)
